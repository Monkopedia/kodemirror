...
- `:lang-python`'s indentation `ContextTracker` is no longer declared `strict = false` (#323).
  ...
  with `@lezer/python` 1.1.18 on 11 before this change (36 with the `advanceStack` fix alone) and
  on none after, while a second run over the first 39 files of that same sorted listing with the
  size filter removed measured 25 before and 16 after. Both runs compare against 1.1.18, the
  version the module's states table was transcribed from at the time, so they record where the port
  stood then rather than making a standing claim about it. Those are one enumeration sampled twice,
  not two independent corpora, and 8 of the 16 are files the 40 also contains and reports clean —
  so the remaining gap is not explained by which files were picked, and its cause is not yet
  established (#357). `elif`/`else` branches, dedented bodies and nested functions were the visible
  casualties.
