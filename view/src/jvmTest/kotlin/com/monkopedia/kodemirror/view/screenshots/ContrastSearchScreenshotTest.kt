...
class ContrastSearchScreenshotTest {

    @Test
    fun capture() = runDesktopComposeUiTest(width = 800, height = 600) {
        setContent {
            val state = remember {
                val initial = EditorState.create(
                    EditorStateConfig(
                ...
                val transaction = initial.update(
                    TransactionSpec(
                        effects = listOf(
                            toggleSearchPanel.of(true),
                            setSearchQuery.of(
                                SearchQuery(search = "fibonacci")
                            )
                        )
                    )
                )
        ...
        onRoot().captureScreenshot("screenshots/compose/contrast-search.png")
    }
}
