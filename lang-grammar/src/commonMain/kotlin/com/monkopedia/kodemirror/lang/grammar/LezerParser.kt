...
// Specializer maps
private val specAtName = mapOf(
    "@asciiLetter" to 244,
    "@asciiUpperCase" to 244,
    "@asciiLowerCase" to 244,
    "@digit" to 244,
    "@whitespace" to 244,
    "@eof" to 244,
    "@specialize" to 76,
...
val lezerParser: LRParser = LRParser.deserialize(
    ParserSpec(
        version = 14,
        ...
        nodeNames = "\u26A0 LineComment BlockComment Grammar RuleDeclaration RuleName " +
            "] [ Props Prop AtName Name = Literal . } { PropEsc ParamList Body " +
            "CharSet AnyChar InvertedCharSet ScopedName Call ArgList CharClass " +
            "? Optional * Repeat + Repeat1 InlineRule ) ( ParenExpression " +
            "Specialization @specialize @extend Sequence PrecedenceMarker ! " +
            "PrecedenceName AmbiguityMarker ~ Choice | RuleDeclaration @top " +
            "PrecedenceDeclaration @precedence PrecedenceBody Precedence " +
            "@left @right @cut TokensDeclaration @tokens TokensBody " +
            "TokenPrecedenceDeclaration PrecedenceBody TokenConflictDeclaration " +
            "@conflict ConflictBody LiteralTokenDeclaration " +
            "LocalTokensDeclaration @local tokens TokensBody ElseToken @else " +
            "ExternalTokensDeclaration @external from TokensBody Token " +
            "ExternalPropDeclaration prop as ExternalPropSourceDeclaration " +
            "propSource ExternalSpecializeDeclaration extend specialize " +
            "ContextDeclaration @context DialectsDeclaration @dialects " +
            "DialectBody TopSkipDeclaration @skip SkipScope SkipBody " +
        ...
        nodeProps = listOf(
            listOf(
                ...
                -16, 5, 13, 20, 21, 22, 23, 24, 26, 28, 30, 32, 33, 36, 37,
                40, 46, "Expression",
                -16, 48, 50, 57, 60, 62, 65, 66, 72, 77, 80, 82, 85, 87, 90,
                92, 94, "Declaration"
            ),
            listOf("openedBy", 6, "[", 15, "{", 34, "("),
            listOf("closedBy", 7, "]", 16, "}", 35, ")")
        ),
        propSources = listOf(lezerHighlighting),
        ...
        tokenizers = listOf(0, 1),
        topRules = mapOf("Grammar" to listOf(0, 3)),
        specialized = listOf(
            SpecializerSpec(
                term = 10,
                get = { value, _ -> specAtName[value] ?: -1 }
            ),
            SpecializerSpec(
                term = 125,
