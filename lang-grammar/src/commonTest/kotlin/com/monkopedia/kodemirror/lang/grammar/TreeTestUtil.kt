...
/**
 * Renders [tree] as a compact `Parent(Child,Child)` string, dropping punctuation-only
 * node types so an assertion reads as the grammar's structure rather than its tokens.
 * Error nodes are rendered as `⚠`.
 */
