...
internal const val COMMENT_TAG_START = 2
internal const val TAG_START = 3
internal const val TEXT = 155
internal const val ENDRAW_TAG_START = 4
internal const val RAW_TEXT = 156
...
/**
 * Base tokenizer: handles text, {{ interpolation starts, {# comment starts, and {% tag starts.
 */
internal val base = ExternalTokenizer({ input, _ ->
    ...
    while (true) {
        ...
        if (next == CH_BRACE_L) {
            ...
            if (after == CH_BRACE_L) {
            ...
            } else if (after == CH_HASH) {
                if (input.pos > start) break
                input.acceptToken(COMMENT_TAG_START, 2)
                return@ExternalTokenizer
                ...
                while (true) {
                    ...
                    if (n == CH_SPACE || n == CH_NEWLINE) {
                        ...
                        input.acceptToken(TAG_START, size)
                        return@ExternalTokenizer
                    }
                }
            }
