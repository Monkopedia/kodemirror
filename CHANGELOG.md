...
### Removed
- Removed the vestigial `IndentContext.textAfterPos` stub from `:language`'s `Indent.kt` — a public property whose getter always returned `""` with no callers anywhere in the repo (the real text-after-position lives in `TreeIndentContext.textAfter`). Also dropped a dead `val unit` computation in `indentString` and fixed its KDoc, which promised tab handling the body never did (it always uses spaces). The ignored `bias` parameters on `lineAt`/`lineIndent`/`column` were left in place because language modules (`lang-python`, `lang-yaml`) still pass explicit non-default `bias` values at their call sites. Removing `textAfterPos` is an intentional public-API removal (`apiDump` refreshed) (#161).
