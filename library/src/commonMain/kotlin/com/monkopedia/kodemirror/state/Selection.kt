...
class SelectionRange private constructor(
    ...
    fun map(change: ChangeDesc, assoc: Side = Side.NEG): SelectionRange {
        ...
        return if (from == this.from && to == this.to) this
        else SelectionRange(from!!, to!!, this.flags)
    }
...
//        }
        ...
        /// @internal
        fun create(from: Int, to: Int, flags: Int): SelectionRange {
            return SelectionRange(from, to, flags)
        }
    }
}
...
private constructor(
    ...
    fun asSingle(): EditorSelection {
        return if (this.ranges.size == 1) this else EditorSelection(listOf(this.main), 0)
    }
...
//            if (!json || !Array.isArray(json.ranges) || typeof json . main != "Int" || json.main >= json.ranges.length)
...
//        }
        ...
        fun single(anchor: Int, head: Int = anchor): EditorSelection {
            return EditorSelection(listOf(EditorSelection.range(anchor, head)), 0)
        }

        /// Sort and merge the given set of ranges, creating a valid
        /// selection.
        fun create(ranges: List<SelectionRange>, mainIndex: Int = 0): EditorSelection {
            if (ranges.isEmpty()) throw IllegalArgumentException("A selection needs at least one range")
            var pos = 0
        ...
        fun cursor(
            ...
            return SelectionRange.create(
                pos,
                pos,
                (if (assoc == Side.ZERO) 0 else if (assoc == Side.NEG) RangeFlag.AssocBefore.mask else RangeFlag.AssocAfter.mask) or
                    (bidiLevel?.coerceAtMost(6) ?: 7) or
                    ((goalColumn
                        ?: RangeFlag.NoGoalColumn.mask) shl RangeFlag.GoalColumnOffset.mask)
            )
        }
        ...
        fun range(
            ...
            return if (head < anchor) SelectionRange.create(
                head,
                anchor,
                RangeFlag.Inverted.mask or RangeFlag.AssocAfter.mask or flags
            ) else SelectionRange.create(
                anchor,
                head,
        ...
        internal fun normalized(ranges: List<SelectionRange>, mainIndex: Int = 0): EditorSelection {
            ...
            while (i < ranges.size) {
                ...
                if (if (range.empty) range.from <= prev.to else range.from < prev.to) {
                    ...
                    ranges[i] = if (range.anchor > range.head) EditorSelection.range(
                        to,
                        from
                    ) else EditorSelection.range(from, to)
                    ranges.removeAt(--i)
                }
                i++
            }
...
fun checkSelection(selection: EditorSelection, docLength: Int) {
    for (range in selection.ranges) {
        if (range.to > docLength) throw IllegalArgumentException("Selection points outside of document")
    }
}
