...
internal fun joinRanges(
    ...
    while (true) {
        ...
        } else if (result[result.size - 1].second < to) result[result.size - 1] =
...
internal fun mergeTransaction(a: ResolvedSpec, b: ResolvedSpec, sequential: Boolean): ResolvedSpec {
    val mapForA =
        if (sequential) b.changes
        else b.changes.map(a.changes)
    val mapForB: ChangeDesc =
        if (sequential) ChangeSet.empty(b.changes.length)
        else a.changes.mapDesc(b.changes, true)
    val changes =
        if (sequential) a.changes.compose(b.changes)
        else a.changes.compose(mapForA)
    return ResolvedSpecData(
        changes,
        selection = b.selection?.map(mapForB) ?: a.selection?.map(mapForA),
        effects = (StateEffect.mapEffects(a.effects.list, mapForA)
            + StateEffect.mapEffects(b.effects.list, mapForB)).list,
...
internal fun filterTransaction(tr: Transaction): Transaction {
    ...
    val filteredTransaction = result.fold(
        ...
        },
        {
            ...
            Transaction.create(
                state, changes, tr.selection?.map(back),
                StateEffect.mapEffects(tr.effects.list, back).list,
                tr.annotations, tr.scrollIntoView
            )
        }
