...
class Annotation<T> internal constructor(
    ...
    companion object {
        ...
        fun <T> define(): AnnotationType<T> = AnnotationType<T>()
    }
}
...
class StateEffect<Value> internal constructor(
    ...
    fun map(mapping: ChangeDesc): StateEffect<Value>? =
        when (val mapped = this.type.map?.invoke(this.value, mapping)) {
            null -> null
            this.value -> this
    ...
    fun <T> isOf(type: StateEffectType<T>): Boolean = this.type == type
    ...
    companion object {
        ...
        fun <Value> define(spec: StateEffectSpec<Value>): StateEffectType<Value> =
            StateEffectType(spec.map ?: { a: Value, _: ChangeDesc -> a })

        fun <T> define(): StateEffectType<T?> = StateEffectType { v, _ -> v }

        fun <Value> define(
            map: ((value: Value, mapping: ChangeDesc) -> Value)? = null
        ): StateEffectType<Value> = define(StateEffectSpec(map))
...
sealed interface Transactable {
    fun isNotEmpty(): Boolean =
        (changes ?: selection ?: effects ?: annotations ?: userEvent) != null ||
            scrollIntoView ||
            !filter ||
            sequential
